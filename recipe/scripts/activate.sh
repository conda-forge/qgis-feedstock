...
elif [ -d $CONDA_PREFIX/Library/qgis/python ]; then
    # On Windows
    export PYTHONPATH="$CONDA_PREFIX/Library/qgis/python:$PYTHONPATH"

    # Windows needs more paths...
    if [[ -n "$QGIS_PREFIX_PATH" ]]; then
        export _CONDA_SET_QGIS_PREFIX_PATH=$QGIS_PREFIX_PATH
    fi
    export QGIS_PREFIX_PATH=$CONDA_PREFIX/Library/qgis

    if [[ -n "$QT_PLUGIN_PATH" ]]; then
        export _CONDA_SET_QT_PLUGIN_PATH=$QT_PLUGIN_PATH
    fi
    export QT_PLUGIN_PATH="$CONDA_PREFIX/Library/plugins:$CONDA_PREFIX/Library/qgis/qtplugins;$QT_PLUGIN_PATH"
fi
